// GET /api/production/providers/higgsfield/callback
// OAuth redirect target for Higgsfield's authorization server. Validates the
...
//
// This route is reached by the user's browser directly from Higgsfield, so
// it cannot carry a Mika admin token — the state check is what protects it.

import {
  completeHiggsfieldAuthorization, buildHiggsfieldCallbackUrl, listHiggsfieldTools, classifyHiggsfieldAuthError,
} from '../../../../../lib/higgsfield/higgsfieldMcpClient';

function redirectWithStatus(res, status, reason) {
  const params = new URLSearchParams({ higgsfield_oauth: status });
...
export default async function handler(req, res) {
  ...
  if (error) {
    const classified = classifyHiggsfieldAuthError(String(error), errorDescription ? String(errorDescription) : '');
  ...
  try {
    await completeHiggsfieldAuthorization({
      redirectUrl,
  ...
  try {
    await listHiggsfieldTools(redirectUrl);
  } catch (e) {
