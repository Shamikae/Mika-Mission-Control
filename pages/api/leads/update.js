// POST /api/leads/update
// Body: { leadId, ...fieldsToUpdate }

import fs   from 'fs';
import path from 'path';

const LEADS_FILE = path.join(process.cwd(), 'data', 'leads.json');

const UPDATABLE = [
  'fullName', 'company', 'email', 'source', 'laneId', 'interestedOfferId',
  'status', 'leadScore', 'notes', 'nextAction', 'assignedAgent',
];

function readStore() {
  try {
    const raw = JSON.parse(fs.readFileSync(LEADS_FILE, 'utf8'));
    return Array.isArray(raw) ? { leads: raw } : raw;
  } catch {
    return { leads: [] };
  }
}

function writeStore(store) {
  fs.writeFileSync(LEADS_FILE, JSON.stringify({ ...store, updatedAt: new Date().toISOString() }, null, 2));
}

export default function handler(req, res) {
  if (req.method !== 'POST') return res.status(405).json({ error: 'Method not allowed' });

  const { leadId, ...updates } = req.body || {};
  if (!leadId) return res.status(400).json({ error: 'leadId is required' });

  const store = readStore();
  const idx   = (store.leads || []).findIndex(l => l.leadId === leadId);
  if (idx === -1) return res.status(404).json({ error: 'Lead not found' });

  const patch = {};
  for (const key of UPDATABLE) {
    if (updates[key] !== undefined) {
      patch[key] = key === 'leadScore'
        ? Math.min(100, Math.max(0, Number(updates[key]) || 0))
        : updates[key];
    }
  }

  store.leads[idx] = { ...store.leads[idx], ...patch, updatedAt: new Date().toISOString() };
  writeStore(store);

  return res.status(200).json({ ok: true, lead: store.leads[idx] });
}
