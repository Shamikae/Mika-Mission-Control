...
export const ADAPTER_ENV_MAP = {
  ...
  'hermes':     {
    required:       ['HERMES_ENABLED', 'HERMES_API_URL'],
    optional:       ['HERMES_CHAT_MODE'],
    activationNote: 'Set HERMES_ENABLED=true and HERMES_API_URL to your VPS Hermes endpoint',
  },
  'claude-code':{
...
export async function runAdapterHealthCheck(adapterId) {
  ...
  const { health, reason, message } = mapToHealthState(adapter, checkResult, envAnalysis);
...
export function persistHealthResults(results) {
