...
import {
  getCanonicalHermesMode,
  recordHermesRuntimeResult,
  sanitizeHermesError,
} from './health';
...
export async function sendHermesChatMessage(message, options = {}) {
  const { mode } = getCanonicalHermesMode();
  let result;

  if (mode === 'http') result = await sendHermesHttpMessage(message, options);
  else if (mode === 'ssh') result = await sendViaSSH(message, options.session || null);
  else result = await sendViaCLI(message, options.session || null);

  recordHermesRuntimeResult(mode, result);
  return {
    ...result,
    error: result.error ? sanitizeHermesError(result.error) : result.error,
    stderr: result.stderr ? sanitizeHermesError(result.stderr) : result.stderr,
  };
}
