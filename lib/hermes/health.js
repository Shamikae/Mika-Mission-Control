...
async function checkSshHealth() {
  ...
  if (!host || !keyFile) {
    runtimeEvidence = {
      mode: 'ssh', reachable: false, verified: false,
      lastChecked: checkedAt,
      error: 'HERMES_SSH_HOST or HERMES_SSH_KEY not configured',
    };
    return;
  }
  ...
  try {
    await execFileAsync('ssh', sshArgs, { timeout: 12_000, env: process.env });
    runtimeEvidence = {
      mode: 'ssh', reachable: true, verified: true,
      lastChecked: checkedAt, error: null,
    };
  } catch (err) {
    const raw = err.stderr?.trim() || err.message || 'SSH health check failed';
    runtimeEvidence = {
      mode: 'ssh', reachable: false, verified: false,
      lastChecked: checkedAt,
      error: sanitizeHermesError(raw),
    };
  }
}
...
export async function getHermesHealthSummary({ checkHttp = false } = {}) {
  ...
  if (mode === 'ssh' && checkHttp) {
    await checkSshHealth();
  }
