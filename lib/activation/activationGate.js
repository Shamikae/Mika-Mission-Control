...
function preflightMaxAgeMs() {
  const configured = Number(process.env.ACTIVATION_PREFLIGHT_MAX_AGE_MS);
  return Number.isInteger(configured) && configured >= 30_000
    ? configured
    : DEFAULT_PREFLIGHT_MAX_AGE_MS;
}
...
function checkHealth(adapterId) {
  ...
  if (!health || !health.checkedAt) {
    return { id: 'health_check', label: 'Health Check', status: 'fail', detail: 'No runtime health evidence. Run a health check in Agent Control first.' };
  }
  const checkedAt = Date.parse(health.checkedAt);
  const ageMs = Number.isFinite(checkedAt) ? Date.now() - checkedAt : Number.POSITIVE_INFINITY;
  if (ageMs > preflightMaxAgeMs()) {
    return { id: 'health_check', label: 'Health Check', status: 'fail', detail: 'Runtime health evidence is stale. Run a new health check.' };
  }
  const bad = ['offline', 'auth_failed', 'misconfigured', 'staged', 'maintenance'];
  if (bad.includes(health.health)) {
    return { id: 'health_check', label: 'Health Check', status: 'fail', detail: `Last check reported ${health.health}.` };
  }
  if (health.health === 'warning' && health.reason === 'configured_but_staged' && health.ok === true) {
    return { id: 'health_check', label: 'Health Check', status: 'pass', detail: 'Adapter is reachable and ready for governed activation.' };
  }
  if (health.health === 'warning') {
    return { id: 'health_check', label: 'Health Check', status: 'fail', detail: 'Last check was degraded or unverified.' };
  }
...
function checkNoDuplicate(adapterId, ignoreRequestId = null) {
...
export function runPreflightChecklist(adapterId, { ignoreRequestId = null } = {}) {
  const items = [
    checkEnvVars(adapterId),
    checkHealth(adapterId),
    checkCostRisk(adapterId),
    checkMaintenance(adapterId),
    checkNoDuplicate(adapterId, ignoreRequestId),
  ];
...
export function approveActivationRequest(requestId) {
  ...
  const storedCheckedAt = Date.parse(req.preflight?.checkedAt || '');
  const storedAgeMs = Number.isFinite(storedCheckedAt)
    ? Date.now() - storedCheckedAt
    : Number.POSITIVE_INFINITY;

  if (
    req.preflight?.adapterId !== req.adapterId
    || req.preflight?.passed !== true
    || storedAgeMs > preflightMaxAgeMs()
  ) {
    return {
      ok: false,
      error: 'Activation approval blocked: the stored preflight is missing, failed, stale, or mismatched.',
      code: 'preflight_not_approvable',
    };
  }

  const approvalPreflight = runPreflightChecklist(req.adapterId, { ignoreRequestId: req.id });
  if (!approvalPreflight.passed) {
    return {
      ok: false,
      error: 'Activation approval blocked: the current preflight did not pass.',
  ...
  req.reviewedBy = 'mika';
  req.approvalPreflight = approvalPreflight;
  saveActivationRequests(store);
