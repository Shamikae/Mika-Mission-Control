...
export const OUTCOME_STATUSES = ['pending', 'approved', 'started', 'completed', 'failed', 'cancelled', 'corrected', 'cache_hit', 'cache_miss'];

// `confirmed_*` means the number is real. `provisional_*` means it is a guess
// and must never be presented as spend. Mirrors the vocabulary the narration
// service and HyperFrames adapter already use.
export const ESTIMATE_TYPES = [
  'confirmed_local',        // local compute — genuinely $0
  'confirmed_provider',     // provider returned an exact pre-flight price
  'provisional_tier',       // cost tier only, no figure
  'provisional_adapter',    // adapter's own static estimate
  'unknown',
];
...
function normalizeMoney(m, { requireConfirmedFlag = true } = {}) {
  if (!m || typeof m !== 'object') {
    return requireConfirmedFlag ? { amount: null, currency: null, confirmed: false } : null;
  }
  return {
    amount: num(m.amount),
