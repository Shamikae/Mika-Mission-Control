...
import { buildRenderIntent } from '../../content/renderIntentSchema';
...
export function createPackageFromWorkforceRun(run, request) {
  ...
  // Render intent — the directorial output (camera/motion/transitions, pacing,
  // visual style, per-scene image + negative prompts, typography, thumbnail art
  // direction, caption variants) that buildContentPackage's written-content
  // schema has no home for and that packaging previously dropped on the floor.
  // Purely additive: a new optional top-level key, verbatim, no reinterpretation
  // and no invented defaults. Readers that predate it are unaffected.
  const renderIntent = buildRenderIntent(stageId => getEffectiveStageOutput(run, stageId), { runId: run.id });
