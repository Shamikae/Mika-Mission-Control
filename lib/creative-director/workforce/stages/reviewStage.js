...
- If the research was live-search mode: cross-check the script's factual claims against the listed evidence. Flag as a blocking issue any statistic or specific claim in the script that is NOT backed by "supported" evidence, any conflicting evidence the script resolved silently instead of preserving the caveat, or any unresolved/unsupported claim the script stated as fact. Also check provenance integrity — the script should not read out raw source IDs unless the brief asked for it.
...
export function buildUserMessage(context) {
  ...
  const lines = [
    ...
    '',
    `RESEARCH — mode: ${research?.researchMode || 'model-synthesis'}`,
  ...
  if (research?.researchMode === 'live-search') {
    lines.push(
      `RESEARCH — supported evidence: ${(research.evidence || []).filter(e => e.verificationStatus === 'supported').map(e => e.claim).join(' | ') || 'none'}`,
      `RESEARCH — conflicting evidence: ${(research.evidence || []).filter(e => e.verificationStatus === 'conflicting').map(e => e.claim).join(' | ') || 'none'}`,
      `RESEARCH — unresolved/unsupported claims: ${(research.unresolvedClaims || []).join(' | ') || 'none'}`,
    );
  }
