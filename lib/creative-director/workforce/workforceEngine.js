...
function buildContext(run, request, { stageId, researchMode, overrideBudget } = {}) {
  const context = {
    request: {
  ...
  for (const wfStageId of WORKFORCE_STAGE_IDS) {
    context[wfStageId] = getEffectiveStageOutput(run, wfStageId);
  }
  if (stageId === 'research') {
    // Defaults to model-synthesis — live search is always an explicit
    // opt-in (never silently triggered by a generic one-click run), since
    // it is the only stage in the whole workforce that can incur real
    // third-party spend outside the OpenRouter model-call budget.
    context.researchOptions = {
      workforceRunId: run.id,
      requestedMode: researchMode === 'live-search' ? 'live-search' : 'model-synthesis',
      overrideBudget: overrideBudget === true,
    };
  }
  return context;
}
 ...
 */
export async function executeOneStage(runId, stageId, { overrideBudget = false, researchMode } = {}) {
  ...
  const context = buildContext(run, request, { stageId, researchMode, overrideBudget });
...
export async function runAllRemaining(runId, { overrideBudget = false, researchMode } = {}) {
  ...
  while (true) {
    ...
    if (!stageId) break;
    run = await executeOneStage(runId, stageId, { overrideBudget, researchMode: stageId === 'research' ? researchMode : undefined });
...
export async function rerunStage(runId, stageId, { overrideBudget = false, note = null, researchMode } = {}) {
  if (!isValidStageId(stageId)) throw new WorkforceError(400, 'invalid_stage', `Unknown stage "${stageId}".`);
  const run = getWorkforceRun(runId);
  if (!run) throw new WorkforceError(404, 'run_not_found', `Workforce run "${runId}" not found.`);
  ...
  return executeOneStage(runId, stageId, { overrideBudget, researchMode });
}
