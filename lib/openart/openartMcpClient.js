...
// Exported for reuse by lib/production/execution/adapters/openartVideoMcp.adapter.js
// — the SAME proven response-parsing logic the real, working image pipeline
// (generateOpenArtImage) already uses, never duplicated/rewritten. This file's
// own image flow (generateOpenArtImage, resolvePrompt, waitForOpenArtCreation,
// etc.) is otherwise untouched.
export function extractHistoryId(normalized) {
...
export function extractCreationStatus(json) {
  return firstDefined(json?.status, json?.creation?.status, json?.data?.status) || null;
}

export function extractPollAfterSeconds(json) {
...
export function collectResourceUrls(normalized) {
  const urls = new Set();
