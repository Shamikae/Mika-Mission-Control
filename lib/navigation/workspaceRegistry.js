...
// Legacy operational destinations retain a canonical workspace plus an
// optional subview. This metadata is one-way and never aliases a canonical
// route back to a legacy route.
export const legacyOperationalRouteMap = Object.freeze({
  approvals: Object.freeze({ workspaceId: 'mission-control', subview: 'priority-actions' }),
  telegram: Object.freeze({ workspaceId: 'mission-control', subview: 'priority-actions' }),
  queue: Object.freeze({ workspaceId: 'mission-control', subview: 'priority-actions' }),
  'activation-gate': Object.freeze({ workspaceId: 'diamond-control', subview: 'activation' }),
  costs: Object.freeze({ workspaceId: 'diamond-control', subview: 'costs' }),
  'cost-intelligence': Object.freeze({ workspaceId: 'diamond-control', subview: 'costs' }),
  'agent-control-center': Object.freeze({ workspaceId: 'diamond-control', subview: 'agent-control' }),
  'agent-dispatch': Object.freeze({ workspaceId: 'diamond-control', subview: 'dispatch' }),
  'adapter-health': Object.freeze({ workspaceId: 'diamond-control', subview: 'system-health' }),
  engineering: Object.freeze({ workspaceId: 'diamond-control', subview: 'engineering' }),
  'engineering-division': Object.freeze({ workspaceId: 'diamond-control', subview: 'engineering' }),
  'task-dispatch': Object.freeze({ workspaceId: 'pipeline', subview: 'dispatch' }),
});
...
export const legacyAliasMap = Object.freeze({
  ...
  ...Object.fromEntries(
    Object.entries(legacyOperationalRouteMap).map(([legacyId, route]) => [legacyId, route.workspaceId]),
  ),
});
...
function validateRouteRegistry() {
  ...
  for (const [legacyId, route] of Object.entries(legacyOperationalRouteMap)) {
    if (legacyAliasMap[legacyId] !== route.workspaceId) {
      throw new Error(`Operational route "${legacyId}" is out of sync with its canonical alias`);
    }
    if (!route.subview) {
      throw new Error(`Operational route "${legacyId}" must define a subview`);
    }
  }
}
...
export function resolveWorkspaceRoute(sectionId) {
  const operationalRoute = legacyOperationalRouteMap[sectionId];
  if (operationalRoute) {
    return {
      sectionId: operationalRoute.workspaceId,
      subview: operationalRoute.subview,
  ...
  return {
    ...
    subview: null,
    legacySectionId: legacyAliasMap[sectionId] ? sectionId : null,
  };
}
