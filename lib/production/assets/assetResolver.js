 ...
 */
export function planSceneAsset(spec, sceneIndex, { capability, brandId, modelOverride, providerOverride } = {}) {
  const warnings = [];
  ...
  //
  // `providerOverride` is forwarded as an OPAQUE string, exactly like
  // modelOverride: this module does not know which providers exist, cannot
  // validate the value, and never branches on it. Policy alone decides whether
  // the named provider has a binding for this capability, and rejects it if not.
  const decision = recommendBinding(planned.request, { modelOverride, providerOverride });
