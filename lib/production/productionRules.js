 ...
 */
export function evaluateAssetReadiness(pkg, { mode, providerId, catalog = PROVIDER_CATALOG }) {
  ...
  for (const key of requiredKeys) {
    ...
    if (ASSET_CHECKS[key].check(pkg)) available.push(key); else missingRequired.push(key);
  }
  for (const key of optionalKeys) {
    if (!ASSET_CHECKS[key]) continue;
    if (ASSET_CHECKS[key].check(pkg)) { if (!available.includes(key)) available.push(key); } else missingOptional.push(key);
  }
