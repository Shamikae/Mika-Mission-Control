...
// ── Provider catalog ──────────────────────────────────────────────────────
// `status` here is the honest DEFAULT. Four entries are patched at runtime
// from real adapter health checks — see resolveLiveCatalog() in
// buildProductionPlan.js: heygen-mcp (lib/production/heygenMcpAvailability.js),
// higgsfield-mcp (lib/production/higgsfieldMcpAvailability.js),
// openart-video (lib/production/openartVideoMcpAvailability.js), and
// hyperframes (lib/production/hyperframesAvailability.js — a local CLI
// health check, not an MCP/OAuth one). Every other provider's status is a
// fixed, documented governance decision: heygen/higgsfield (the old,
// staged, Direct-API entries) have adapter *files* but their execute()
...
export const PROVIDER_CATALOG = [
  ...
  },
  {
    ...
    status: 'staged', // live-patched from real OAuth/tool-discovery health — see resolveLiveCatalog() in buildProductionPlan.js, same pattern as heygen-mcp/openart-video
    supportedModes: ['cinematic_broll', 'product_demo', 'image_to_video'],
    requiredInputs: ['visualBrief'],
    optionalInputs: ['durationSeconds'],
    supportsAvatar: false,
    supportsReferenceImage: false, // reference-image input exists in Higgsfield's live schema but is not implemented in this checkpoint — see higgsfieldMcp.adapter.js
    supportsVoice: false,
    supportsCaptions: false,
    supportsProjectFiles: false,
    estimatedCostTier: 'variable', // a real, non-generating cost preflight exists (get_cost) — see estimate() — but it is model/prompt-dependent, never a fixed tier
    executionType: 'mcp-oauth',
    activationRequirements: [
      'Set HIGGSFIELD_MCP_ENABLED=true and complete OAuth connect in the Higgsfield Connection panel',
      'Select an image or video model from the live Higgsfield model catalog in Higgsfield Setup',
    ],
  },
  {
    id: 'hyperframes',
    displayName: 'HyperFrames',
    status: 'staged', // live-patched from real CLI-runnable + composition-discovery health — see resolveLiveCatalog() in buildProductionPlan.js, same pattern as heygen-mcp/higgsfield-mcp/openart-video
    // Broad catalog modes — composition CONTENT varies per composition and
    // isn't introspectable from the catalog level; the real gate is
    // explicit composition selection (hyperframes.adapter.js's
    // validateInput() requires and live-verifies compositionId).
    supportedModes: ['cinematic_broll', 'faceless_social', 'product_demo'],
    requiredInputs: ['compositionId'],
    optionalInputs: ['quality'],
    supportsAvatar: false,
    supportsReferenceImage: false, // no reference-media input in this checkpoint
    supportsVoice: false,
    supportsCaptions: false,
    supportsProjectFiles: false, // composition selection is from EXISTING local compositions only — no upload path
    estimatedCostTier: 'free', // real, confirmed $0 — local CPU/GPU time only, no provider credits (see hyperframes.adapter.js's estimate())
    executionType: 'local-cli',
    activationRequirements: [
      'Set HYPERFRAMES_ENABLED=true (or leave unset — enabled by default, no credentials required)',
      'The HyperFrames CLI must be runnable (npx hyperframes --version) and at least one valid composition must exist under tools/hyperframes/',
    ],
  },
  {
    id: 'openart-video',
    displayName: 'OpenArt Video',
    status: 'staged', // live-patched from real OAuth/tool-discovery health — see resolveLiveCatalog() in buildProductionPlan.js, same pattern as heygen-mcp/higgsfield-mcp
    // text2video needs no reference media, exactly like Higgsfield's cinematic_broll/
    // product_demo. image_to_video and "slideshow" are deliberately excluded:
    // image_to_video would require a real reference-media path (image2video/
    // element2video — not implemented this checkpoint, see
    // openartVideoMcp.adapter.js), and no OpenArt tool/schema evidence of a
    // "slideshow" capability was ever found in live discovery.
    supportedModes: ['cinematic_broll', 'product_demo'],
    requiredInputs: ['visualBrief'],
    optionalInputs: ['durationSeconds'],
    supportsAvatar: false,
    supportsReferenceImage: false, // image2video/element2video exist in OpenArt's live schema but are not implemented in this checkpoint — see openartVideoMcp.adapter.js
    supportsVoice: false,
    supportsCaptions: false,
    supportsProjectFiles: false,
    estimatedCostTier: 'variable', // a real, non-generating cost preflight exists (openart_model_cost) — see estimate() — but it is model/duration/resolution-dependent, never a fixed tier
    executionType: 'mcp',
    activationRequirements: [
      'Set OPENART_ENABLED=true and complete OAuth connect in the OpenArt Connection panel',
      'Select a text2video-capable model from the live OpenArt model catalog in OpenArt Video Setup',
    ],
  },
  {
    id: 'manual-export',
