...
function ledgerContextFor(job) {
  ...
  return {
    ...
    estimate: {
      ...
      estimateType: budget.estimateType === 'confirmed_local' ? 'confirmed_local'
        : budget.estimateType === 'confirmed_provider' ? 'confirmed_provider'
...
async function finalizeCompletion(job, pkg, rawResult, { actor, token }) {
  ...
  recordLedger(current, 'execution_completed', {
    ...
    actual: normalized.actualCost && Number.isFinite(normalized.actualCost.amountUsd)
      ? { amount: normalized.actualCost.amountUsd, currency: 'USD', confirmed: true }
      : (isPaidJob(current) ? { amount: null, currency: null, confirmed: false }
                            : { amount: 0, currency: 'USD', confirmed: true }),
  });
