 ...
 */
function markExecutionFailed(job, { reason, message, actor = 'system', token = null }) {
...
async function executeSubmission(job, pkg, lockResult, { actor }) {
  ...
  if (!submitResult.ok) {
    const failed = markExecutionFailed(current, { reason: submitResult.errorReason || 'provider_error', message: submitResult.error || 'Provider submit failed.', actor: 'system', token });
    return { ok: true, job: failed };
  }
