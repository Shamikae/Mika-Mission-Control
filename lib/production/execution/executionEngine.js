...
import { COST_ESTIMATE_TYPES, normalizeCost } from '../../cost/costShape.js';
import { appendLedgerEntry } from '../../ledger/ledgerStore.js';
import { isPaidExecution } from '../../ledger/ledgerRules.js';
...
// ── Ledger integration (F2) ──────────────────────────────────────────────
// THE single point where this engine emits Ledger records. Provider adapters
// know nothing about the Ledger and must never import it — every governed
// execution is recorded here, from data the engine already holds.
//
// FAILURE POLICY, stated explicitly because it governs money:
//   • PAID execution — a failed ledger write BLOCKS. Unledgered spend is
//     never permitted, so `recordLedger` is called BEFORE submit for the
//     started event and its result is checked by the caller.
//   • FREE execution (costTier 'free' / estimateType 'confirmed_local' —
//     HyperFrames, manual-export) — a failed ledger write DEGRADES: it is
//     surfaced on the job's activity history and execution continues. There
//     is no spend to lose track of, and refusing to render locally because
//     of a bookkeeping failure would be a worse outcome than the gap.
function ledgerContextFor(job) {
  const budget = job?.budget || {};
  return {
    division: job?.metadata?.division || 'content',
    capability: job?.metadata?.capability || job?.selectedMode || 'video_render',
    source: {
      packageId: job?.packageId || null,
      productionJobId: job?.id || null,
      assetRequestId: job?.metadata?.assetRequestId || null,
      renderSpecId: job?.metadata?.renderSpecId || null,
      sceneId: job?.metadata?.sceneId ?? null,
    },
    binding: {
      providerId: job?.selectedProvider || null,
      model: job?.providerInput?.model || null,
      executionType: job?.metadata?.executionType || null,
    },
    estimate: {
      amount: budget.estimatedRange?.min ?? (budget.costTier === 'free' ? 0 : null),
      currency: budget.currency || 'USD',
      // The unit is passed through exactly as the job recorded it, never
      // inferred from the currency string and never converted. A budget that
      // declares no unit stays unitless in the record rather than being
      // silently promoted to dollars.
      unit: budget.unit ?? null,
      providerCreditUnit: budget.providerCreditUnit ?? null,
      isLowerBound: budget.isLowerBound === true,
      // Any term from the SHARED vocabulary is preserved exactly — a
      // catalogue price stays identifiable as one rather than being flattened
      // to a generic tier. Only the legacy alias and the free-tier default are
      // translated; anything unrecognised still degrades to provisional_tier.
      estimateType: COST_ESTIMATE_TYPES.includes(budget.estimateType) ? budget.estimateType
        : budget.estimateType === 'provisional_adapter_estimate' ? 'provisional_adapter'
    ...
    approval: {
      required: job?.approval?.required === true,
      approvalRef: job?.approval?.approvedAt ? job.id : null,
      approvedAt: job?.approval?.approvedAt || null,
      approvedBy: job?.approval?.approvedBy || null,
    },
  };
}

/**
 * Normalizes an adapter-reported actual cost. Returns null when the adapter
 * reported nothing usable, so the caller can fall back to "unknown" rather
 * than to a fabricated zero.
 */
function actualCostFrom(reported) {
  if (!reported || typeof reported !== 'object') return null;
  // Legacy USD-only shape, still honoured.
  if (Number.isFinite(reported.amountUsd)) {
    return { amount: reported.amountUsd, unit: 'currency', currency: 'USD', confirmed: true };
  }
  if (!Number.isFinite(reported.amount)) return null;
  const cost = normalizeCost({ ...reported, estimateType: 'confirmed_provider', confirmed: true });
  if (!cost.unit) return null;
  return {
    amount: cost.amount,
    unit: cost.unit,
...
function isPaidJob(job) {
  const budget = job?.budget || {};
  return isPaidExecution({
    costTier: budget.costTier,
    estimateType: budget.estimateType,
    amount: budget.estimatedRange?.min,
  });
}

/**
 * Emits one ledger record for a lifecycle transition.
 * @returns {{ ok: boolean, id?: string, error?: string, blocking: boolean }}
 */
function recordLedger(job, event, { actor = 'system', outcome = {}, actual } = {}) {
  const ctx = ledgerContextFor(job);
  const result = appendLedgerEntry({
    ...
    outcome: {
      ...
      completedAt: job?.execution?.completedAt || null,
      ...outcome,
    },
  });
  return { ...result, blocking: !result.ok && isPaidJob(job) };
}
...
function markExecutionFailed(job, { reason, message, actor = 'system', token = null, rawMetadata } = {}) {
  ...
  const failedRecord = updateProductionJob(job.id, {
    status: 'failed',
    execution: {
      ...job.execution,
      status: 'failed',
  ...
  applyProductionRefToPackage(failedRecord);
  // Terminal outcome — always recorded. A failed ledger write here cannot
  // block anything (the execution already ended); it is surfaced by the
  // caller-visible result of appendLedgerEntry, never swallowed silently.
  recordLedger(failedRecord, 'execution_failed', {
    actor,
    outcome: { status: 'failed', errorReason: reason || 'unknown_error' },
  });
  return failedRecord;
}
...
async function finalizeCompletion(job, pkg, rawResult, { actor, token }) {
  ...
  // Terminal outcome. `actual` is only asserted when the adapter reported a
  // genuinely confirmed figure — otherwise it stays unknown rather than being
  // back-filled from the estimate.
  recordLedger(current, 'execution_completed', {
    actor,
    outcome: {
      status: 'completed',
      artifactIds: savedOutputs.map(o => o.id).filter(Boolean),
    },
    // An adapter may report real spend in ANY unit — a vendor's own credits as
    // readily as dollars. This previously accepted only `amountUsd`, so every
    // provider that bills in credits had its confirmed actual cost silently
    // discarded and recorded as unknown. The unit is taken from what the
    // adapter declared and is never converted.
    actual: actualCostFrom(normalized.actualCost)
      || (isPaidJob(current) ? { amount: null, currency: null, confirmed: false }
                             : { amount: 0, unit: 'currency', currency: 'USD', confirmed: true }),
  });
...
async function executeSubmission(job, pkg, lockResult, { actor }) {
  ...
  // Ledger BEFORE submit: for paid work an unwritable ledger must stop the
  // spend, not discover it afterwards.
  const startLedger = recordLedger(current, 'execution_started', { actor, outcome: { status: 'started' } });
  if (startLedger.blocking) {
    const failed = markExecutionFailed(current, {
      reason: 'validation_error',
      message: `Refusing paid execution: the Ledger entry could not be written (${startLedger.error}). Unledgered spend is not permitted.`,
      actor: 'system', token,
    });
    return { ok: true, job: failed };
  }
  if (!startLedger.ok) {
    current = updateProductionJob(job.id, {
      activityHistory: [...current.activityHistory, makeActivityEvent('execution_progress', {
        actor: 'system',
        note: `Ledger write failed and was degraded (free execution): ${startLedger.error}`,
      })],
    });
  }
...
export async function cancelExecutionForJob(jobId, { actor = 'user', note } = {}) {
  ...
  if (isQueued(jobId) && (!job.execution || job.execution.status === 'queued')) {
    ...
    recordLedger(cancelled, 'execution_cancelled', { actor, outcome: { status: 'cancelled' } });
