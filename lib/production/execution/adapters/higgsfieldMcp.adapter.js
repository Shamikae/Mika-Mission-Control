  ...
  async estimate({ job }) {
    ...
    if (!providerInput?.model || !providerInput?.prompt || !MEDIA_TYPES.includes(providerInput?.mediaType)) {
      return {
        ...
        // Declares the UNIT this provider bills in. Never a currency: no
        // Higgsfield API documents a credit-to-dollar rate, so a figure here
        // must never be added to, or compared with, a dollar price.
        unit: 'provider_credits', providerCreditUnit: 'higgsfield-credits',
        provisional: true, approvalRequired: true,
