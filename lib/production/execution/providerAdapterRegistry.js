...
//
// Four adapters are registered: manual-export (always executable),
// mock-video (executable only when explicitly enabled), heygen-mcp, and
// higgsfield-mcp (both executable only when OAuth-authenticated with all
// required live tools discovered — see each adapter's healthCheck()).
// Every other Production Router provider catalog entry (higgsfield,
// hyperframes, openart-video, heygen-api) has NO execution adapter — they
// remain staged/unavailable and are reported honestly via
// listProviderExecutionStatus(). Note "higgsfield" (the old, Direct-API-
// staged agent-domain entry) and "higgsfield-mcp" (this real adapter)
// coexist deliberately, exactly mirroring "heygen" vs "heygen-mcp".
