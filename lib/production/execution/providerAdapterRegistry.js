...
//
// Five adapters are registered: manual-export (always executable),
...
// see each adapter's healthCheck()). Every other Production Router
// provider catalog entry (higgsfield, hyperframes, heygen-api) has NO
// execution adapter — they remain staged/unavailable and are reported
// honestly via listProviderExecutionStatus(). Note "higgsfield" (the old,
// Direct-API-staged agent-domain entry) and "higgsfield-mcp" (the real
// adapter) coexist deliberately, exactly mirroring "heygen" vs "heygen-mcp".
