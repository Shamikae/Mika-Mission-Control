...
export const NON_RETRYABLE_ERROR_REASONS = new Set([
  ...
  // OpenArt Video (openartVideoMcp.adapter.js): the account genuinely lacks
  // the credits or plan entitlement required — retrying the exact same
  // request would fail identically every time, so retrying is never useful
  // and only risks confusing repeated attempts.
...
export function checkExecutionEligibility(job, pkg, { providerExecutable, providerKnown } = {}) {
  ...
  if (!pkg) {
    ...
    // The package's `production` block describes its RENDER — the deliverable
    // job whose artifact becomes the video. This guard stops a superseded
    // render from executing.
    //
    // An Asset Generation job is a different class: it produces an INPUT the
    // render later consumes, never claims the package's production pointer,
    // and can legitimately run while a completed render still holds it. It is
    // therefore exempt — scoped by division, not by provider.
    const isAssetJob = job.metadata?.division === 'asset-generation';
    if (!isAssetJob && pkg.production?.latestJobId && pkg.production.latestJobId !== job.id) {
