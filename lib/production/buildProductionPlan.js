...
import { resolveOpenArtVideoMcpStatus } from './openartVideoMcpAvailability';
import { resolveHeyGenMcpStatus } from './heygenMcpAvailability';
import { resolveHiggsfieldMcpStatus } from './higgsfieldMcpAvailability';
import { resolveHyperFramesStatus } from './hyperframesAvailability';
...
/**
 * Clones PROVIDER_CATALOG with openart-video/heygen-mcp/higgsfield-mcp's
 * status/activationRequirements patched from live MCP tool discovery, and
 * hyperframes's from a real local CLI-runnable + composition-discovery
 * check (no MCP/OAuth involved). Every other entry is a fixed, documented
 * governance decision (see productionRules.js).
 */
async function resolveLiveCatalog() {
  const [openArtLive, heygenLive, higgsfieldLive, hyperframesLive] = await Promise.all([
    resolveOpenArtVideoMcpStatus(), resolveHeyGenMcpStatus(), resolveHiggsfieldMcpStatus(), resolveHyperFramesStatus(),
  ]);
  return PROVIDER_CATALOG.map(p => {
    ...
    if (p.id === 'hyperframes') {
      return { ...p, status: hyperframesLive.status, activationRequirements: hyperframesLive.status === 'active' ? [] : [...p.activationRequirements, hyperframesLive.reason] };
    }
    return p;
  });
}
...
export async function buildProductionJob({
  ...
  // An EXPLICITLY requested provider is honoured, full stop. It is only when no
  // provider was asked for that the recommendation is used.
  //
  // This previously required the requested provider to appear in PROVIDER_CATALOG
  // and SILENTLY SUBSTITUTED the recommended one otherwise. That catalog drives
  // VIDEO render recommendations, so an image-only provider legitimately absent
  // from it (Asset Generation binds those) was swapped out while its
  // provider-specific `providerInput` — including the model name — was kept.
  // The result was a job addressed to one vendor carrying another vendor's
  // model: a silent vendor substitution inside a spend path, which is the one
  // failure mode that must never be quiet.
  //
  // Executability is NOT decided here and never was: checkExecutionEligibility()
  // gates on the execution adapter registry (providerKnown/providerExecutable)
  // before anything runs, so an unrecognised provider still fails — visibly,
  // with its own name in the error, instead of being replaced.
  const finalProviderId = selectedProvider || providerRec.recommendedProvider;
