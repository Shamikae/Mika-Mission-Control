...
async function resolveLiveCatalog() {
  ...
  return PROVIDER_CATALOG.map(p => (
    p.id === 'openart-video'
      ? { ...p, status: live.status, activationRequirements: live.status === 'active' ? [] : [...p.activationRequirements, live.reason] }
      : p
  ));
}
...
export async function buildProductionJob({
  packageId, selectedMode, selectedProvider, maxEstimatedCost, currency, approvalRequiredAbove,
  ...
  const readiness = evaluateAssetReadiness(pkg, { mode: selMode, providerId: finalProviderId, catalog });
