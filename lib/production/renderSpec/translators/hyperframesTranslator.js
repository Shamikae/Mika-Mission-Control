...
// Resolved scene images are copied in under a translator-owned filename.
// The extension comes from an allowlist, never from a caller-supplied string.
const IMAGE_MIME_EXT = { 'image/png': 'png', 'image/jpeg': 'jpg', 'image/webp': 'webp' };

function sceneImageFilename(sceneIndex, mimeType) {
  const ext = IMAGE_MIME_EXT[mimeType];
  if (!ext || !Number.isFinite(sceneIndex)) return null;
  return `scene-${sceneIndex}.${ext}`;
}
...
export function planTranslation(spec, { narration = null } = {}) {
  ...
  // ── Scenes / timeline ──────────────────────────────────────────────────
  // Resolved assets from URS. Consumed by index only — the translator never
  // learns who generated them.
  const assetsByScene = new Map(
    (Array.isArray(spec?.assets) ? spec.assets : [])
      .filter(a => Number.isFinite(a?.sceneIndex) && IMAGE_MIME_EXT[a?.mimeType])
      .map(a => [a.sceneIndex, a]),
  );
  ...
  const scenes = usable.map((sc, i) => {
    ...
    return {
      ...
      // Fixed, translator-owned filename — never a caller-supplied path.
      // Populated only when this scene has a resolved asset.
      imageFile: assetsByScene.has(i) ? sceneImageFilename(i, assetsByScene.get(i).mimeType) : null,
      // Asset IDENTITY, not just presence. The filename is positional
      // (scene-0.png) and would be identical for any image, so without these
      // two fields swapping the underlying asset would silently reuse the same
      // composition hash — and therefore the same cached composition.
      imageAssetId: assetsByScene.has(i) ? (assetsByScene.get(i).assetId || null) : null,
      imageContentHash: assetsByScene.has(i) ? (assetsByScene.get(i).contentHash || null) : null,
  ...
  const resolvedCount = scenes.filter(s => s.imageFile).length;
  if (resolvedCount > 0) {
    consume('assets[].sceneVisual');
  }
  if (resolvedCount < scenes.length && scenes.some(s => s.visualDescription || s.generationPrompt)) {
    warnings.push(`${scenes.length - resolvedCount} of ${scenes.length} scenes render as styled placeholders — their visual descriptions and generation prompts remain metadata, not imagery.`);
  }
...
function buildScenesMarkup(scenes) {
  return scenes.map((s) => {
    ...
    return [
      ...
      '        <div class="scene-bg"><div class="placeholder"></div><div class="scene-image"></div></div>',
...
/**
 * True when a directory holds no composition at all — no manifest AND no
 * index.html. That is debris from an interrupted write, not a hand-authored
 * composition, and must not permanently block its own content hash.
 *
 * The overwrite guard exists to protect real hand-authored work; an empty
 * shell is not that, and treating it as such made a composition id
 * un-generatable forever.
 */
function isEmptyDebrisDir(dir) {
...
export function translateUrsToHyperFrames(spec, { dryRun = false, narration = null } = {}) {
  ...
  if (fs.existsSync(targetDir)) {
    if (!isGeneratedDir(targetDir) && !isEmptyDebrisDir(targetDir)) {
  ...
  try {
    ...
    // Resolved scene images are copied in beside the narration, under the
    // translator's own filename. The SOURCE path comes from the URS asset
    // manifest and is re-validated to stay inside the project; the
    // DESTINATION is always assets/scene-<n>.<ext>.
    for (const scene of data.scenes) {
      if (!scene.imageFile) continue;
      const asset = assetsByScene.get(scene.index);
      if (!asset?.storagePath) continue;
      if (path.isAbsolute(asset.storagePath) || asset.storagePath.includes('..')) {
        throw new Error('Refusing an asset storagePath that is absolute or traverses.');
      }
      const src = path.resolve(process.cwd(), asset.storagePath);
      if (!src.startsWith(process.cwd() + path.sep)) {
        throw new Error('Refusing an asset source outside the project root.');
      }
      const assetsDir = path.join(stagingDir, 'assets');
      fs.mkdirSync(assetsDir, { recursive: true });
      fs.copyFileSync(src, path.join(assetsDir, scene.imageFile));
    }
