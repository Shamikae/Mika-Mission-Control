...
// Calls openartAdapter.execute() to get image Buffers, then saves them via
// saveImageArtifact. Returns a standard result shape with imageFiles[].
...
async function executeViaOpenArt(task, decision) {
  ...
  try {
    ...
    return {
      ...
      selectedSkill:   { agentId: 'openart', skillId: null, loaded: false, engine: 'http-api' },
      output:          null,
      outputSummary:   null,
      error:           errMsg,
  ...
  // Save image bytes to persistent artifact storage
  let imageFiles = [];
  try {
    ...
    return {
      ...
      errorSummary:    errMsg.slice(0, 120),
      httpStatus:      null,
      rawResponse:     { model: adapterResult.model || null },
      imageFiles:      [],
      decision,
      timestamp:       new Date().toISOString(),
    };
  ...
  return {
    ...
    errorSummary:    null,
    httpStatus:      null,
    rawResponse:     { model: adapterResult.model || null, imageCount: count },
    imageFiles,
    decision,
    timestamp:       new Date().toISOString(),
  };
