...
export const useStore = create((set, get) => ({
  ...
  appendChatMessage: (agentId, msg) => set((s) => {
    const existing = s.agentChats[agentId] || [];
    if (msg.id && existing.some(m => m.id === msg.id)) return s;
    return { agentChats: { ...s.agentChats, [agentId]: [...existing, msg] } };
  }),
  ...
  // ── Operating Mode (ADHD) ────────────────────────────
  operatingMode: typeof window !== 'undefined'
    ? (localStorage.getItem('mika-mode') || 'normal')
    : 'normal',
  setOperatingMode: (mode) => {
    if (typeof window !== 'undefined') {
      try { localStorage.setItem('mika-mode', mode); } catch {}
    }
