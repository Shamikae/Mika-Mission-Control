...
export async function fetchGatewayStatus() {
  try {
    ...
    return {
      ...
      version: null,
      lastPing: null,
      unavailableReason: 'OpenClaw status could not be loaded.',
    };
  }
}
...
export async function fetchHermesHttpStatus() {
  try {
    ...
    if (!res.ok) return { mode: 'cli', configured: false, reachable: false, verified: false, status: 'unknown' };
    return res.json();
  } catch { return { mode: 'cli', configured: false, reachable: false, verified: false, status: 'unknown' }; }
}
...
export async function dispatchTaskToHermes(taskId) {
  const result = await executeViaDispatch(taskId);
  return {
    ...result,
    success: result.ok,
    status: result.task?.status || result.executionStatus,
    hermesOutput: result.executionTarget === 'hermes' ? result.output : null,
    hermesError: result.error,
  };
}
...
export async function verifyTelegramDelivery() {
  ...
  const body = await res.json().catch(() => ({}));
  if (!res.ok) {
    throw new Error(body.error || 'Telegram verification request failed.');
  }
...
export async function dispatchTaskToOpenClaw(taskId) {
  ...
  return {
    ...
    success: result.ok,
    status: result.task?.status || result.executionStatus,
    openclawReply: result.executionTarget === 'openclaw' ? result.output : null,
    openclawError: result.error,
  };
}
...
export async function restartAgent(agentId) {
  if (MOCK_MODE) return { success: false, agentId, error: 'Agent restart is not connected.' };
  return gw(`/api/agents/${agentId}/restart`, { method: 'POST' });
}

export async function stopAgent(agentId) {
  if (MOCK_MODE) return { success: false, agentId, error: 'Agent stop is not connected.' };
...
export async function fetchGoals() {
  return {
    ...
    message: 'Goal history is not loaded because no safe vault read source is configured.',
  };
}

export async function updateGoalProgress(goalId, current) {
  if (MOCK_MODE) return { success: false, error: 'Goal update is not connected.' };
...
export async function fetchJournalEntries() {
  return {
    ...
    source: 'unavailable',
    status: 'configuration_pending',
    message: 'Journal history is unavailable. New entries can still be appended to the configured vault.',
  };
}

export async function createJournalEntry(entry) {
  if (MOCK_MODE) return { success: false, error: 'Legacy journal creation is not connected.' };
...
export async function fetchMemoryVault(query = '') {
  try {
    const response = await fetch('/api/vault/graph', { cache: 'no-store' });
    if (!response.ok) throw new Error('Vault index request failed');
    ...
    if (payload.warn) {
      return {
        ...
        source: 'unavailable',
        status: 'configuration_pending',
        message: 'Vault memory is unavailable or not configured.',
      };
    }

    const normalizedQuery = query.trim().toLowerCase();
    const items = (payload.nodes || [])
      .filter(node => !node.isTag)
      .map(node => ({
        id: node.id,
        title: node.name,
        category: node.group || 'Vault',
        content: null,
        tags: [],
    ...
    return {
      ...
      source: 'vault_index',
      status: 'available',
      message: 'Real Obsidian note index. Note contents and filesystem paths are not exposed.',
    };
  } catch {
    return {
      items: [],
      source: 'unavailable',
      status: 'unavailable',
      message: 'Vault memory could not be loaded.',
    };
  }
}
...
export async function fetchPrompts() {
  return {
    ...
    source: 'unavailable',
    status: 'configuration_pending',
    message: 'Prompt Library has no safe configured read source.',
  };
}
...
export async function fetchContentTasks() {
  return {
    ...
    source: 'unavailable',
    status: 'configuration_pending',
    message: 'No governed content-task source is configured for this legacy view.',
  };
}
...
export async function fetchLeadMetrics() {
  return {
    source: 'unavailable',
    status: 'configuration_pending',
    message: 'Legacy aggregate lead metrics are not connected.',
  };
}
...
export async function fetchCannaOpsData() {
  return {
    source: 'unavailable',
    status: 'configuration_pending',
    message: 'CannaOps operational telemetry is not connected.',
  };
}
...
export async function fetchMedAIData() {
  return {
    source: 'unavailable',
    status: 'configuration_pending',
    message: 'MedAI operational telemetry is not connected.',
  };
}
